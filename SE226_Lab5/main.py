...
def replace(word):
    nList = list(word)
    rand = random.randint(0, len(nList) - 1)
    nList[rand] = nList[rand].upper()
    return "".join(nList)
...
def swap(word):
    ...
    tail = len(nList)
    first = nList[0:2]
    last = nList[tail - 2:tail]
    mid = nList[2:tail - 2]
    currentpassword = last + mid + first
...
temp = int(input("Please enter a number between 3 and 7:"))
while temp < 3 or temp > 7:
    print("You have entered invalid value.")
    temp_number = int(input("Please enter a number between 3 and 7:"))
else:
    temp = generator(temp)
    print("Initial password:  " + temp)
    temp = replace(temp)
    temp = swap(temp)
    print("Final version of the password:  " + temp)
    search(temp, "h")
